...
onMounted(() => {
  ...
  mobile.value =
    ...
    /iPhone|iPod|Android.*Mobile/i.test(userAgent);
  standalone.value =
    window.matchMedia("(display-mode: standalone)").matches ||
    navigatorWithHints.standalone === true;

  const storedNextPromptAt = Number(localStorage.getItem(nextPromptKey) || 0);
  if (Number.isFinite(storedNextPromptAt))
    nextPromptAt.value = storedNextPromptAt;
  ...
  >
    <template #header>
      ...
      <h2 id="pwa-install-title">
        {{ iosInstructionsVisible ? "在 iPhone 加入主畫面" : "把江映澄紫微加入主畫面" }}
      </h2>
    </template>
    <div class="install-sheet-content">
      <template v-if="iosInstructionsVisible">
        <p>
          在 Safari
          下方工具列點選「分享」，向下捲動後選擇「加入主畫面」，再點右上角的「加入」。
        </p>
      </template>
      <template v-else>
        <p>
          加入主畫面後，可像 App
          一樣快速開啟，並在網路不穩時繼續查看已儲存的內容。
        </p>
      </template>
      <div class="install-sheet-actions">
        ...
        <button
          class="app-button"
          type="button"
          @click="handlePrimaryInstallAction"
        >
  ...
  <Transition name="install-banner">
    ...
    >
      <span class="prompt-icon"
        ><RefreshCw v-if="$pwa?.needRefresh" :size="22" /><Smartphone
          v-else
          :size="23"
      /></span>
...
.install-sheet-icon {
  ...
  height: 58px;
  margin: 4px auto 2px;
  border: 1px solid rgba(107, 166, 160, 0.25);
  border-radius: 19px;
...
.install-sheet-actions .app-button {
  width: 100%;
  gap: 6px;
}
.pwa-prompt {
  position: fixed;
  z-index: calc(var(--layer-navigation) + 2);
  right: 50%;
  ...
  background: linear-gradient(
    ...
    rgba(107, 166, 160, 0.16)
  );
  box-shadow: 0 14px 34px rgba(36, 87, 90, 0.18);
  color: var(--mountain);
  -webkit-backdrop-filter: blur(22px) saturate(150%);
  backdrop-filter: blur(22px) saturate(150%);
}
.pwa-prompt.without-action {
  grid-template-columns: 44px minmax(0, 1fr) 34px;
}
.pwa-prompt.without-action .prompt-close {
  grid-column: 3;
}
.prompt-icon {
  display: grid;
...
.prompt-close {
  ...
  height: 31px;
  padding: 0;
  border: 1.5px solid rgba(36, 87, 90, 0.3);
  border-radius: 50%;
...
@media (min-width: 760px) and (max-width: 1179px) {
  .pwa-prompt {
    right: calc((100vw - 88px) / 2);
    bottom: 20px;
    width: min(652px, calc(100vw - 124px));
  }
}
...
@media (max-width: 420px) {
  ...
  .install-sheet-actions .app-button:first-child {
    order: 2;
  }
  .pwa-prompt {
