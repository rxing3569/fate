...
const cities = [...cityData, { id: 'OTHER', name: '其他（手動輸入經度）', country: 'OTHER', lng: 120 }] as CityOption[]
...
const filtered = computed(() => {
  ...
  return cities.filter(city => city.name.toLowerCase().includes(keyword) || city.id.toLowerCase().includes(keyword) || city.country.toLowerCase().includes(keyword))
})
      ...
      <Transition name="sheet">
        <div v-if="open" class="sheet-backdrop content-sheet-backdrop city-backdrop" @click.self="open = false">
          <section class="city-sheet" role="dialog" aria-modal="true" aria-labelledby="city-picker-title">
            ...
            <div class="city-list">
              <button v-for="city in filtered" :key="city.id" type="button" :class="{ selected: city.id === normalizedValue }" @click="choose(city)">
                <span class="city-pin"><MapPin :size="18" /></span>
                <span><strong>{{ city.name }}</strong><small>{{ city.id === 'OTHER' ? '搜尋不到時可自行輸入經度' : `${city.id} · 經度 ${city.lng.toFixed(2)}°` }}</small></span>
...
.city-list { flex:1;overflow:auto;margin-top:10px;padding:0 2px;overscroll-behavior:contain;scrollbar-color:rgba(36,87,90,.28) transparent; }.city-list::-webkit-scrollbar { width:5px; }.city-list::-webkit-scrollbar-track { background:transparent; }.city-list::-webkit-scrollbar-thumb { border-radius:99px;background:rgba(36,87,90,.28); }.city-list button { display:grid;grid-template-columns:42px 1fr 24px;gap:10px;align-items:center;width:100%;min-height:66px;padding:9px 10px;border:0;border-radius:16px;background:transparent;color:var(--mountain);text-align:left; }.city-list button:hover,.city-list button.selected { background:rgba(107,166,160,.12); }.city-pin { display:grid;place-items:center;width:38px;height:38px;border-radius:50%;background:rgba(36,87,90,.09); }.city-list strong,.city-list small { display:block; }.city-list strong { font-size:14px; }.city-list small { margin-top:4px;color:var(--text-soft);font-size:11px; }.city-list p { color:var(--text-soft);text-align:center; }
</style>
