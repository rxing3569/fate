...
<template>
  <div class="city-picker">
    ...
    <Teleport to="body">
      ...
      >
        <template #header>
          <header class="city-sheet-header"><span><LocateFixed :size="21" /><strong id="city-picker-title">選擇出生城市</strong></span><button type="button" aria-label="關閉城市選擇" @click="open = false"><X :size="21" /></button></header>
        </template>
...
.city-sheet-body { display:flex;flex:1;min-height:0;flex-direction:column; }:global(.city-sheet.app-bottom-sheet) { height:min(75dvh,720px);padding:12px 16px calc(12px + env(safe-area-inset-bottom)); }:global(.city-sheet-content.app-bottom-sheet-content) { display:flex;flex:1;min-height:0;flex-direction:column;overflow:hidden;text-align:left; }.city-sheet-header { display:flex;align-items:center;justify-content:space-between;padding:0 4px 12px; }.city-sheet-header > span { display:flex;align-items:center;gap:9px; }.city-sheet-header button { display:grid;place-items:center;width:40px;height:40px;border:0;border-radius:50%;background:rgba(36,87,90,.08);color:var(--mountain); }
