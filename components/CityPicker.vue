...
const selected = computed(() =>
  cities.find((city) => city.id === normalizedValue.value),
);
const filtered = computed(() => {
  const keyword = query.value.trim().toLowerCase();
  if (!keyword) return cityData as CityOption[];
  return (cityData as CityOption[]).filter(
    (city) =>
      city.name.toLowerCase().includes(keyword) ||
      city.id.toLowerCase().includes(keyword) ||
...
<template>
  <div class="city-picker">
    ...
    >
      ...
      <span v-if="selected"
        ...
        ><small>{{
          selected.id === "OTHER"
            ? "手動設定出生地經度"
            : `${selected.id} · 經度 ${selected.lng.toFixed(2)}°`
        }}</small></span
      >
      <span v-else class="city-placeholder"
        ><strong>請選擇出生城市</strong></span
      >
    ...
    >
      <template #header>
        <header class="city-sheet-header">
          <LocateFixed :size="21" /><strong id="city-picker-title"
            >選擇出生城市</strong
          >
        </header>
      </template>
      <div class="city-sheet-body">
        ...
        <div class="city-other">
          <button
            class="city-option"
            type="button"
            :class="{ selected: normalizedValue === 'OTHER' }"
            @click="choose(otherCity)"
          >
            <span class="city-pin"><MapPin :size="18" /></span>
            <span
              ><strong>{{ otherCity.name }}</strong
              ><small>搜尋不到時可自行輸入經度</small></span
            >
            <Check
              v-if="normalizedValue === 'OTHER'"
              :size="20"
              aria-hidden="true"
            />
          </button>
        ...
        <div class="city-list" data-sheet-scroll>
          <button
            ...
            class="city-option"
            type="button"
            :class="{ selected: city.id === normalizedValue }"
            @click="choose(city)"
          >
            <span class="city-pin"><MapPin :size="18" /></span>
            <span
              ...
              ><small>{{
                `${city.id} · 經度 ${city.lng.toFixed(2)}°`
              }}</small></span
            >
            <Check
              v-if="city.id === normalizedValue"
              :size="20"
              aria-hidden="true"
            />
          </button>
...
:global(.city-sheet.app-bottom-sheet) {
  padding-inline: 16px;
}
:global(.city-sheet-content.app-bottom-sheet-content) {
  display: flex;
  flex: 1;
  min-height: 0;
  flex-direction: column;
