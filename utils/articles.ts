...
const files = import.meta.glob("../data/articles/*.md", {
  query: "?raw",
  import: "default",
  eager: true,
...
function parseArticle(path: string, raw: string): Article {
  ...
  const content =
    leadingHeading?.[1]?.trim() === title
      ? markdown.slice(leadingHeading[0].length).trim()
      : markdown;
  const nextStepVariant = ["intro", "stars", "fortune"].includes(
    metadata.nextStepVariant,
  )
  ...
  return {
    slug,
    title,
    fileName: metadata.fileName || slug,
    excerpt: metadata.excerpt || "",
    category: metadata.category || "紫微入門",
    date: metadata.date || "",
    readingTime: metadata.readingTime || "5 分鐘",
    content,
    nextStepVariant,
    seoDescription: metadata.seoDescription || metadata.excerpt || "",
    seoKeywords: (metadata.seoKeywords || "紫微斗數, 紫微解析, 命盤解析")
      .split(",")
      .map((item) => item.trim())
      .filter(Boolean),
  };
