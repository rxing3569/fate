...
const titleFor = (item: HistoryItem) => {
  if (activeKind.value === "quota") {
    return (
      analysisLabel(item.analysis_type) ||
      analysisLabel(item.type) ||
      quotaTypeLabels[normalizeType(item.type)] ||
      "會員額度異動"
    );
  }
...
async function load(reset = false) {
  ...
  try {
    ...
    items.value.push(
      ...(page.items || []).filter((item) => !known.has(item.uuid)),
    );
  ...
  <AppPageLayout title="異動紀錄" screen-class="history-screen" show-back>
    <main class="history-content">
      <div class="history-tabs" role="tablist">
        <button
          :class="{ active: activeKind === 'points' }"
          type="button"
          @click="activeKind = 'points'"
        >
          <Coins :size="17" />點數紀錄
        </button>
        <button
          :class="{ active: activeKind === 'quota' }"
          type="button"
          @click="activeKind = 'quota'"
        >
          <Gauge :size="17" />額度紀錄
        </button>
      </div>
      ...
      <aside class="retention-note">
        <Coins :size="18" /><span
          ><strong>紀錄僅保存一年</strong
          ><small>系統以月份為單位保留最近 12 個月紀錄。</small></span
        >
      </aside>

      <div v-if="loading" class="history-state">
        <span class="loading-ring" />正在讀取紀錄…
      </div>
      <div v-else-if="error && !items.length" class="history-state error">
        <strong>讀取紀錄失敗</strong><span>{{ error }}</span
        ><button class="app-button outline" type="button" @click="load(true)">
          重新嘗試
        </button>
      </div>
      <div v-else-if="!items.length" class="history-state">
        <span class="empty-icon"
          ><Coins v-if="activeKind === 'points'" :size="32" /><Gauge
            v-else
            :size="32" /></span
        ><strong
          >尚無{{ activeKind === "points" ? "點數" : "額度" }}異動紀錄</strong
        >
      </div>
      ...
      <section v-else class="history-list">
        ...
        >
          <span class="transaction-icon" :class="{ positive: item.amount > 0 }"
            ><ShoppingBag v-if="item.type === 'purchase'" :size="23" /><Gift
              v-else-if="item.type === 'reward' || item.type === 'refund'"
              :size="23" /><Gauge
              v-else-if="activeKind === 'quota'"
              :size="23" /><Sparkles v-else :size="23"
          /></span>
          <div class="transaction-copy">
            <div>
              <strong>{{ titleFor(item) }}</strong
              ><b :class="{ positive: item.amount > 0 }"
                >{{ item.amount > 0 ? "+" : "" }}{{ item.amount }}
                {{ activeKind === "points" ? "P" : "次" }}</b
              >
            </div>
          ...
          <small class="balance-after">{{
            activeKind === "points"
              ? `異動後 ${item.balance_after ?? 0} P`
              : `剩餘 ${item.quota_remaining_after ?? 0} 次`
          }}</small>
        </article>
      </section>
      <p v-if="error && items.length" class="inline-error">{{ error }}</p>
      <div ref="sentinel" class="history-sentinel">
        <span v-if="loadingMore" class="loading-ring" /><small
...
<style scoped>
.history-screen {
  background: linear-gradient(180deg, var(--paper), rgba(107, 166, 160, 0.1));
}
.history-content {
  padding: 10px 18px calc(42px + env(safe-area-inset-bottom));
}
.history-tabs {
  display: grid;
...
.history-tabs button.active {
  ...
  box-shadow: 0 6px 14px rgba(36, 87, 90, 0.18);
  color: #fff;
}
.retention-note {
  display: flex;
  ...
  border-radius: 16px;
  background: rgba(180, 155, 117, 0.11);
  color: var(--mountain);
}
.retention-note span {
  display: grid;
...
.retention-note small {
  color: var(--text-soft);
  font-size: 11px;
}
.range-filter {
  display: grid;
  grid-template-columns: 1fr auto 1fr 40px;
  align-items: end;
  gap: 7px;
  margin-bottom: 16px;
  padding: 12px;
...
.history-state.error span {
  font-size: 12px;
}
.history-state .app-button {
  min-height: 42px;
}
.empty-icon {
  display: grid;
